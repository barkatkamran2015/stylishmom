...
// Map page types to their slug base paths
const pagePaths = {
  Recipe: "/food",
  Drinks: "/drinks",
  Dessert: "/dessert",
...
// Parse thumbnail from content or fallback
const parseThumbnail = (post) => {
  const imageMatch = (post.content || post.post_content || post.body)?.match(
    /<img[^>]+src=["'](.*?)["']/i
  );
  return post.imageUrl || post.image_url || (imageMatch ? imageMatch[1] : "/default-image.jpg");
};

// Helper to sanitize text
const sanitizeText = (html) => (html ? html.replace(/<[^>]+>/g, "").trim() : "");
  ...
  try {
    ...
    if (!response.ok) {
      ...
      return {
        props: {
          initialPosts: [],
          initialPagination: { total: 0, limit, offset, totalPages: 0 },
    ...
    const parsedPosts = posts.map((post) => {
      const thumbnailUrl = parseThumbnail(post);
...
export default function Home({ initialPosts, initialPagination, error: initialError }) {
  ...
  // Navigate directly to the post's slug page
  const navigateToPost = (post) => {
    const path = pagePaths[post.page]
      ? `${pagePaths[post.page]}/${post.slug}`
      : `/blog/${post.slug}`;
    router.push(path);
  };
    ...
    mainEntity: {
      ...
      itemListElement: filteredPosts.map((post, index) => ({
        ...
        item: {
          "@type": "BlogPosting",
          headline: post.title,
          description: sanitizeText(post.contentHtml).substring(0, 160),
    ...
    <div className={styles.homePage}>
      <Head>
        ...
        {pagination.offset > 0 && pagination.offset - pagination.limit >= 0 && (
          <link
            rel="prev"
            href={`https://www.thestylishmama.com/?limit=${pagination.limit}&offset=${
      ...
      {loading ? (
        ...
        <>
          ...
          <Slider {...sliderSettings} className={styles.homePage__featuredSlider}>
            <div>
              <Image src={imageBlog} alt="Blog" width={1200} height={600} priority />
              <h3 className={styles.homePage__sliderText}>Love Does not Divide, It Multiplies</h3>
            </div>
            <div>
              <Image src={imageNature} alt="Natures Beauty" width={1200} height={600} priority />
              <h3 className={styles.homePage__sliderText}>Every family is Unique</h3>
            </div>
            <div>
              <Image src={imageRecipe} alt="Recipe" width={1200} height={600} priority />
                ...
                >
                  ...
                  <div className={styles.homePage__postDetails}>
                    ...
                    <p className={styles.homePage__postExcerpt}>
                      {sanitizeText(post.contentHtml).substring(0, 200)}...
                    </p>

                    <button className={styles.homePage__ctaButton} onClick={() => navigateToPost(post)}>
                      Read More
                    </button>
                  </div>
                </div>
