...
export default function DrinkPost({ post }) {
    ...
    return (
      <div className={styles.drinksPage}>
        <Head>
          ...
          <meta name="description" content="The drink recipe you are looking for could not be found." />
        </Head>
  ...
  const description = excerpt(post.content, 160) || 'Read this drink recipe from The Stylish Mama.';
  const pageUrl = `${baseUrl}/drinks/${encodeURIComponent(post.slug)}`;
      ...
      <section className={styles.heroSection}>
        {post.imageUrl ? (
          <Image
            ...
            priority
            onError={(e) => (e.currentTarget.src = '/default-drinks-image.jpg')}
          />
        ) : (
...
export async function getStaticProps({ params }) {
  ...
  try {
    ...
    const res = await fetch(`${API_URL}?page=Drinks&slug=${slug}`);
    if (res.ok) {
    ...
    const listRes = await fetch(`${API_URL}?page=Drinks&limit=1000&offset=0`);
